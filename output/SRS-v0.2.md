...
| **Structured Output** | LLM이 사전 정의된 JSON Schema에 맞춰 출력을 생성하는 기법. Vercel AI SDK의 `generateObject()` 등으로 구현한다. |
...
| ASS-006 | 외부 가격/뉴스/커뮤니티 데이터 소스는 Next.js `fetch` 캐시(`revalidate`) 또는 `unstable_cache`를 통해 지연 허용형으로 공급된다. |
| ASS-007 | 푸시 발송은 Web Push API + Service Worker 기반으로 Vercel 호스팅 환경에서 동작한다. |
...
|---|---|---|---|---|
| Market Data Provider | 외부 데이터 소스 | OHLCV (시가/고가/저가/종가/거래량) | REST + Next.js `fetch` 캐시 | 지연 허용 범위 내 캐시 사용, 10분 이상 지연 시 경보 |
| News Data Provider | 외부 데이터 소스 | 종목/섹터 관련 뉴스, 감성 점수 | REST + Next.js `fetch` 캐시 | 결측률 3% 초과 시 경보 |
| Community Signal Provider | 외부 데이터 소스 | 소셜/커뮤니티 시그널 점수 | REST + Next.js `fetch` 캐시 | 지연 허용형 캐시 |
| **Google Gemini API** | **LLM 서비스** | **추천 카드 Structured Output (JSON)** | **Vercel AI SDK (`@ai-sdk/google`)** | **Rate limit 준수, Serverless Function timeout 내 처리, `GEMINI_MODEL` 환경 변수로 모델 교체** |
...
| Push Notification Provider | 외부 인프라 | 웹 푸시 발송 | Web Push API + REST | 발송 성공률 ≥ 99%, 발송 예약 시각 대비 5분 이내 |
...
| 클라이언트 | 플랫폼 | 통신 방식 | 비고 |
|---|---|---|---|
| Web App (Next.js) | PC/모바일 브라우저 | Server Components (RSC), Server Actions, Route Handlers | 추천 카드, Confidence Score UI, 성과 카드. Tailwind CSS + shadcn/ui |
| Mobile Web | 모바일 브라우저 | 동일 Next.js 앱 (반응형) + Web Push | 아침 브리핑 푸시 수신, 딥링크 랜딩 |
...
| `/api/events` | POST | Route Handler | 행동 이벤트 추적 수집 (fire-and-forget) | 누락률 < 1% |
| `/api/cron/morning-briefing` | GET | **Vercel Cron** | 아침 브리핑 푸시 배치 발송 (`vercel.json` cron 설정) | 예약 대비 5분 이내 |
| `/api/admin/health` | GET | Route Handler | 데이터 소스 freshness 및 결측률 확인 (운영용) | — |
...
sequenceDiagram
    ...
    participant Action as Server Logic (Route Handler)
    participant DB as Prisma (SQLite/Supabase)
    participant Cache as fetch Cache (Market/News)
    ...
    alt 캐시된 카드 존재
        ...
        Page->>LLM: generateObject() 호출 (watchlist, signals, risk_mode 컨텍스트)
        LLM-->>Page: Structured Output (카드 1~3개 JSON) 또는 에러
        alt LLM 성공
            Page->>DB: RecommendationCard 저장 (status=published)
        else LLM 실패
            Page->>DB: No Call 카드 저장 + llm_call_failed 이벤트 기록
        end
    end
...
sequenceDiagram
    ...
    participant EventAPI as POST /api/events
    ...
    Client->>EventAPI: POST /api/events (event_name=confidence_change)
    EventAPI-->>Client: ACK
```
...
sequenceDiagram
    ...
    loop 대상 사용자별
        ...
        Handler->>DB: NotificationLog 기록 (pushSent 이벤트)
    end
    PushProvider->>Client: 푸시 메시지 수신
    User->>Client: 푸시 탭 (오픈)
    Client->>Client: 딥링크 파싱 및 라우팅
    Client->>User: 홈 또는 상세 화면 표시 (랜딩 지연 ≤ 1,000ms)
    Client->>EventAPI: POST /api/events (push_open, deeplink_success)
    EventAPI-->>Client: ACK
```
...
| REQ-FUNC-021 | 가격 산출값이 0 이하이거나 비정상 급등락 범위로 판정된 카드는 게시되지 않아야 하며, `rec_validation_failed` 이벤트가 기록되어야 한다. | M | Story 3 AC3-2 | **Given** LLM 출력 또는 입력 데이터 이상으로 가격이 0 이하 또는 비정상 범위로 산출되었고 **When** 카드 게시 전 Zod 스키마 검증을 수행하면 **Then** 해당 카드는 게시되지 않고 rec_validation_failed 이벤트가 Prisma를 통해 기록되어야 한다. |
...
| REQ ID | 요구사항 설명 | Priority | Source | Acceptance Criteria |
|---|---|---|---|---|
| REQ-FUNC-050 | 시스템은 Vercel Cron(`/api/cron/morning-briefing`)을 통해 푸시 수신에 동의한 사용자에게 미국장 전 지정 발송 시간 기준 ±5분 이내로 브리핑 푸시를 발송해야 한다. Cron 핸들러는 `CRON_SECRET` 환경 변수로 인증한다. | S | Story 6 AC6-1, PRD §4.1 Should | **Given** 사용자가 푸시 수신에 동의했고 **When** Vercel Cron이 발송 시간에 핸들러를 호출하면 **Then** 푸시는 예약 시각 대비 5분 이내 발송되고 발송 성공률은 99% 이상이어야 한다. |
...
| REQ-FUNC-052 | 푸시 수신을 거부했거나 OS 권한을 회수한 사용자는 발송 대상에서 제외되어야 하며, 잘못 발송된 메시지 비율은 0%이어야 한다. | M | Story 6 AC6-3 | **Given** 사용자가 푸시 수신을 거부했거나 OS 권한을 회수했고 **When** Cron 배치를 실행하면 **Then** 해당 사용자는 Prisma 쿼리(`consentPush: false`)로 발송 대상에서 제외되어야 하며, 잘못 발송된 메시지 비율은 0%이어야 한다. |
...
| REQ ID | 요구사항 설명 | Priority | Source | Acceptance Criteria |
|---|---|---|---|---|
| REQ-FUNC-060 | 시스템은 다음 이벤트를 클라이언트에서 Route Handler(`POST /api/events`)로 전송하고 Prisma를 통해 직접 DB에 기록해야 한다: `home_view`, `rec_card_impression`, `rec_card_click`, `rec_detail_view`, `bookmark_add`, `alert_set`, `broker_redirect`, `price_copy`, `execution_intent_submit`, `confidence_view`, `confidence_change`, `performance_card_view`, `reason_expand`, `push_sent`, `push_open`, `deeplink_success`, `deeplink_fail`, `rec_validation_failed`, `risk_profile_load_result`, `llm_call_failed`. 클라이언트는 실패 시 1회 retry + `navigator.sendBeacon` fallback을 적용한다. | M | PRD §1.1, §1.2, §3.x | **Given** 사용자가 해당 행동을 수행했고 **When** 이벤트가 클라이언트에서 발행되면 **Then** 이벤트는 1% 미만 누락률로 Prisma를 통해 DB에 기록되어야 한다. |
| REQ-FUNC-061 | 시스템은 이벤트 기반으로 NS-01 ADR, SU-01~SU-08 보조 KPI를 일간/주간 집계하여 KPI 대시보드에 제공해야 한다. | S | PRD §1.2, §5.4 | **Given** 이벤트 로그가 수집되고 **When** KPI 집계 배치(Vercel Cron 또는 수동 트리거)가 실행되면 **Then** ADR, 추천 카드 CTR, First Decision Time, Confidence Engagement Rate, D7/D30 Retention 등 정의된 KPI가 집계되어야 한다. |
...
| REQ ID | 요구사항 설명 | Priority | Source | Acceptance Criteria |
|---|---|---|---|---|
| REQ-FUNC-080 | 시스템은 **Vercel AI SDK**(`@ai-sdk/google`)를 통해 **Google Gemini API**를 호출하여 추천 카드를 생성해야 한다. 호출은 Next.js Server Action 또는 Route Handler 내에서 이루어진다. | M | C-TEC-005, C-TEC-006 | **Given** 사용자 watchlist와 시장 데이터가 준비되었고 **When** LLM 호출이 실행되면 **Then** Gemini API가 호출되고 RecommendationCard 스키마를 준수하는 JSON이 반환되어야 한다. |
| REQ-FUNC-081 | LLM 출력은 **Structured Output** (`generateObject()`) 형식으로 강제되어야 하며, 카드 스키마(`ticker`, `direction`, `entryPrice`, `targetPrice`, `holdDays`, `confidenceScore`, `reasonLine`)에 100% 적합해야 한다. 스키마 불일치 시 1회 재시도하고, 실패 시 No Call 상태로 처리한다. | M | C-TEC-005, F2 | **Given** LLM 호출 결과가 반환되었고 **When** Zod 스키마 검증을 수행하면 **Then** 모든 필수 필드가 존재하고 타입이 일치해야 한다. 불일치 시 1회 재시도 후에도 실패하면 No Call 카드를 생성한다. |
...
| REQ-FUNC-083 | LLM 호출 실패(API 에러, timeout, rate limit) 시 시스템은 **No Call 상태 카드**를 반환하고 `llm_call_failed` 이벤트를 Prisma를 통해 기록해야 한다. 사용자에게 에러 페이지가 노출되어서는 안 된다. | M | F2 REQ-FUNC-013 | **Given** Gemini API 호출이 5xx/timeout/rate limit으로 실패했고 **When** 에러 핸들링이 실행되면 **Then** No Call 카드가 반환되고 llm_call_failed 이벤트가 기록되며, 사용자에게는 안내 문구만 표시되어야 한다. |
...
| REQ-FUNC-093 | 사용자 탈퇴 시 관련 데이터(watchlist, riskProfile, eventLog)가 Prisma를 통해 삭제 또는 익명화되어야 한다. | S | GDPR/개인정보 보호 | **Given** 사용자가 탈퇴를 요청했고 **When** 탈퇴 처리 Server Action이 실행되면 **Then** 해당 사용자의 watchlist, riskProfile, eventLog가 삭제 또는 익명화되어야 한다. |
...
| REQ-NF-014 | 이벤트 추적 누락률 | 클라이언트 발행 대비 Prisma DB 적재 실패 < 1.0%. 클라이언트 사이드 1회 retry + `navigator.sendBeacon` fallback 적용. | 일간 | 일 단위 1% 초과 시 데이터 경보 | PRD §5.2 |
| REQ-NF-015 | 푸시 발송 성공률 | 발송 시도 대비 공급자 성공 응답 ≥ 99.0% | 1회 Cron 배치 단위 | 배치당 99% 미만 시 경보 | PRD §5.2, Story 6 |
...
|---|---|---|---|---|---|
| REQ-NF-040 | 제품 KPI 모니터링 | ADR, 카드 CTR, Confidence Engagement Rate, 성과 카드 열람률 실시간 집계 | Vercel Analytics + 커스텀 대시보드 | 기준선 대비 20% 이상 급락 24시간 지속 시 PM/분석 리뷰 | PRD §5.4 |
...
| REQ-NF-043 | 푸시 운영 모니터링 | 발송 성공률, 오픈율, 딥링크 실패율 | 이벤트 로그 기반 | 성공률 99% 미만 또는 딥링크 실패율 1% 초과 시 마케팅/앱 온콜 | PRD §5.4 |
...
| REQ ID | 항목 | 요구사항 | Source |
|---|---|---|---|
| REQ-NF-070 | Serverless Function Timeout | Route Handler 및 Server Action의 실행 시간은 Vercel Plan 제한(Hobby: 10s, Pro: 60s)을 초과하지 않도록 설계한다. LLM 호출이 포함된 함수는 **Vercel AI SDK 스트리밍** 응답(`streamObject()`)을 사용할 수 있다. | C-TEC-001, C-TEC-007 |
...
| PRD §4.1 Must | F7 | REQ-FUNC-060~061 | 이벤트 Prisma 직접 write < 1% 누락, KPI 집계 | TC-FUNC-060, TC-FUNC-061 |
...
| C-TEC-001/007 | — | **REQ-NF-070~073** | **Vercel 런타임 제약 (timeout, cold start, 연결 풀, 번들 사이즈)** | **TC-NF-070~073** |
...
| `/api/events` | POST | Route Handler | `userId`, `recId` (optional), `eventName`, `eventValue` (optional) | `{ ack: true }` | NextAuth.js 세션 필수 | 누락률 < 1% |
| `/api/cron/morning-briefing` | GET | **Vercel Cron** | `CRON_SECRET` 헤더 검증 | `{ scheduled: true, sent: N, failed: N }` | `CRON_SECRET` 환경 변수 | 발송 시각 ±5분, timeout 내 처리 |
...
> **참고:** 모든 모델은 Prisma ORM으로 정의하며, SQLite(로컬)과 PostgreSQL(Supabase) 양쪽에서 호환된다.  
...
| `userId` | String | FK → User.id, NOT NULL | 수신 사용자 |
| `notifType` | String | NOT NULL | 알림 유형 ('morning_briefing' / 'watchlist_alert' 등) |
| `sentAt` | DateTime? | NULLABLE | 발송 일시 |
| `openedAt` | DateTime? | NULLABLE | 사용자 오픈 일시 |
| `deeplinkUrl` | String? | NULLABLE | 딥링크 URL |
| `sendStatus` | String | NOT NULL | 'scheduled' / 'sent' / 'failed' (Zod 검증) |

#### 6.2.8 EventLog

| 필드명 (Prisma) | 타입 | 제약 | 설명 |
|---|---|---|---|
| `id` | String | `@id @default(cuid())` | 이벤트 PK |
| `userId` | String | FK → User.id, NOT NULL | 이벤트 발생 사용자 |
| `recId` | String? | FK → RecommendationCard.id, NULLABLE | 연관 추천 카드 |
| `eventName` | String | NOT NULL | 이벤트명 (예: rec_card_click, broker_redirect, llm_call_failed) |
| `eventValue` | Json? | NULLABLE | 이벤트 부가 데이터 (SQLite: TEXT, PG: JSONB) |
| `occurredAt` | DateTime | `@default(now())` | 이벤트 발생 일시 |
| `sessionId` | String? | NULLABLE | 세션 식별자 |
| `clientType` | String? | NULLABLE | 'web' / 'mobile_web' |
...
sequenceDiagram
    ...
    Handler->>LLM: generateObject() 호출 (watchlist, signals, 3벌 risk_mode)
    LLM-->>Handler: Structured Output JSON (aggressive/balanced/conservative 3벌)
    ...
    alt 검증 통과
        ...
        Handler->>LLM: generateObject() 재시도 (1회)
        LLM-->>Handler: 재시도 결과
        ...
        alt 재검증 통과
            ...
            Handler->>DB: eventLog.create() (eventName='rec_validation_failed')
        end
    end
    ...
    alt LLM 호출 자체 실패 (timeout/rate limit/5xx)
        LLM-->>Handler: 에러 응답
        Handler->>DB: recommendationCard.create() (status='no_call')
        Handler->>DB: eventLog.create() (eventName='llm_call_failed')
    end
```
...
sequenceDiagram
    ...
    Page->>EventAPI: POST /api/events (performance_card_view, reason_expand)
    EventAPI->>DB: eventLog.create()
    EventAPI-->>Page: ACK
```
...
sequenceDiagram
    ...
    participant Client as Client App (Browser + Service Worker)
    actor User
    participant EventAPI as POST /api/events
    ...
    loop 대상 사용자별 (timeout 내 처리)
        ...
        Handler->>PushProvider: 푸시 발송 요청 (딥링크 포함)
        PushProvider-->>Handler: 발송 성공/실패 응답
        Handler->>DB: notificationLog.create({ sendStatus: 'sent' | 'failed' })
    end

    PushProvider->>Client: 푸시 메시지 수신 (Service Worker)
    User->>Client: 푸시 탭
    Client->>Client: 딥링크 파싱 및 Next.js 라우팅
    Client->>User: 홈 또는 상세 화면 렌더링 (랜딩 ≤ 1,000ms)
    Client->>EventAPI: POST /api/events (push_open, deeplink_success 또는 deeplink_fail)
    EventAPI->>DB: eventLog.create()
    EventAPI-->>Client: ACK
```
...
sequenceDiagram
    participant Client as Client Component
    participant EventAPI as POST /api/events (Route Handler)
    participant DB as Prisma (EventLog)
    participant KPICron as Vercel Cron (KPI 집계)
    participant Dashboard as KPI Dashboard

    Client->>EventAPI: POST /api/events (userId, eventName, eventValue)
    EventAPI->>DB: eventLog.create({ ... })
    DB-->>EventAPI: 저장 완료
    EventAPI-->>Client: ACK (HTTP 200)

    alt 전송 실패
        Client->>Client: 1회 retry
        alt retry 실패
            Client->>Client: navigator.sendBeacon() fallback
        end
    end

    Note over KPICron: 일간/주간 배치 집계
    KPICron->>DB: eventLog 쿼리 (ADR, CTR, D7 등 KPI 계산)
    DB-->>KPICron: 집계 결과
    KPICron->>Dashboard: KPI 지표 업데이트
```
